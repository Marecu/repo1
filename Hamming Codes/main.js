...
const descriptions = {
    intro: {
        ...
        By using this search-based method based on the arrangement of parity groups (which we call a Hamming Code), we are able to track a 4 × 4 grid of data while only using 4 of the bits (1 per parity group) for redundancy purposes.`,
        drawbacks: `While Hamming Codes are great at correcting a single error, if two or more bits get flipped, the whole process falls apart. By the nature of using a systematic search that narrows down to one position, it is not possible to track more than one error at a time. A Hamming Code with multiple errors will still point you to a position on the grid, but it won't be the correct one and flipping the bit it points to will only lead to the creation of more errors in the data. This is extremely important to consider and plays a major role in deciding how we apply Hamming Codes to transmit data - this will be discussed further in the "Extending to n × n" section.`,
        squareOne: `If you've been paying close attention, you might have noticed an issue with the first square in the grid. The first square doesn't fall into any of the parity groups. This creates ambiguity - if there's an error in that position, none of the parity groups will be affected and indicate that there is an error present in the grid. Therefore, when we see no errors in the parity groups, it could mean one of two things: either that there is no error, or there is an error in the first grid position. As such, we cannot use this position to store meaningful data since there is no way to correct it. This leaves us with four redundancy bits and eleven data bits, which is called an (11, 4) Hamming Code.<br><br>
        Even if we can't store data with it, we can still put the bit to work. If we set the bit in the first position to make the parity of the <strong>whole grid</strong> even, it acts as a check for multiple errors present in the grid. Recall that the parity of this bit will toggle when any of the bits in the grid are flipped. This means that if we have two errors, the parity of the bit will remain even. Since there are clearly errors created, the parity groups will indicate that there is an error. In cases like this where the parity of the whole grid is unaffected but the parity of individual groups is, we know there must be more than one error. If there is only one error, we'll see that reflected in the parity of the whole grid as well.<br><br>
        Adding this to the grid, we now have what is called an Extended Hamming Code with five redundancy bits and eleven data bits. An example is shown below with the parity bits highlighted in green, data bits highlighted in grey, and the total grid parity bit highlighted in yellow:
        <div class="introImageContainer introImageContainerLarge">
            <div class="introImage introImageLarge" style="background-image: url(assets/completeExample.png);"></div>
        </div>`,
    ...
    bhc: {
        title: "Visualizing",
        description: "Shown here is a 4 × 4 Extended Hamming Code with the parity bits highlighted green, data bits highlighted white, and the total grid parity bit highlighted yellow.\n\nHover over a parity bit to see which portions of the grid are contained within its respective parity group.\n\nClick any square in the grid to flip the bit from a 0 to a 1 or vice versa. When a bit gets flipped, the parity of each group will get calculated. If the parity is odd (meaning there is an error within the parity group), the group's parity bit will turn red.\n\nTry flipping different bits and seeing how each position affects the parity groups in a unique way and how flipping two bits gets caught by the grid parity bit as discussed in the introduction."
    },
    nxn: {
        title: "Scaling",
...
const loadIntro = () => {
    ...
    const populateMainArea = () => {
        ...
        introWrapper.appendChild(makeSection("4 - The First Square", descriptions.intro.squareOne));
